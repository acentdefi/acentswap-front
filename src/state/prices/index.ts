...
export const fetchPrices = createAsyncThunk<PriceApiThunk>('prices/fetch', async () => {
  ...
  const response2 = await fetch('https://api.acentswap.info/api/tokens')
  ...
  const acentAddr = Object.keys(data2.data)[0]
  prices.data[acentAddr.toLowerCase()] = parseFloat(data2.data[acentAddr].price)
